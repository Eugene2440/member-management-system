...
async function loadMembers() {
    ...
    try {
        ...
        const token = localStorage.getItem('adminToken');
        const response = await fetch(`${window.API_URL}/api/members`, {
            headers: {
...
async function updatePaymentStatus(memberId, status) {
    try {
        const token = localStorage.getItem('adminToken');
        const response = await fetch(`${window.API_URL}/api/members/${memberId}/payment`, {
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${token}`,
...
async function saveMember() {
    try {
        ...
        const token = localStorage.getItem('adminToken');
        const response = await fetch(`${window.API_URL}/api/members/${memberId}`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
